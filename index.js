...
const time = moment().tz('Asia/Palu').format("HH:mm:ss")
...
const { BotWa, tanggal, waktu, instagram, whatsapp, youtube, nomer, aktif, ontime } = config
...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] QR code is ready, subscribe KilL4Life ID`)
})
       ...
       case 'wiki':
           wiki(value)
               .then(data => {
                    ...
                    let hasil = `☞︎︎︎Menurut Wikipedia:\n\n${res}`
                    client.sendMessage(id, hasil,MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'covid':
           corona(value)
               .then(data => {
                   ...
                   let hasil = `✔︎DATA WABAH COVID-19 TERBARU DI INDONESIA\n\🤕Positif ==> ${positif} \n😃Sembuh ==> ${sembuh} \n😭Meninggal ==> ${meninggal}`
                   client.sendMessage(id, hasil,MessageType.text)
               })
               .catch(err => {
                   console.log(err)
